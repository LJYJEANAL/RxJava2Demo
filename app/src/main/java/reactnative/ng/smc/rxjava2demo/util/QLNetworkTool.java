...
public class QLNetworkTool {
	private final static String TAG = QLNetworkTool.class.getSimpleName();
	...
	public static String getHost(String url) {
		...
		android.util.Log.i("NetWorkTool", u + "->host=" + host);
		return host;
	}
	...
	public static int getPort(String url, int defaultPort) {
		...
		android.util.Log.i("NetWorkTool", u + "->port=" + port + "  strPort=" + strPort);
		return port;
	}
	...
	public static String getRequestRelative(String url) {
		...
		android.util.Log.i("NetWorkTool", u + "->relativeStr=" + relativeStr);
		return relativeStr;
	}

	/**
	 * @param showToast
	 *            网络不稳定时是否《Toast》提示信息（默认：当前网络不稳定，请稍后在试）
	 * @param showMessage
	 *            网络不稳定时《Toast》提示的信息
	...
	public static boolean isConnectInternet(Context context, boolean showToast, String showMessage) {
		...
		android.util.Log.i(TAG, netSataus ? "当前网络可用" : "当前网络不可用");
		return netSataus;
	}
	...
	public static boolean isWifiOpen(Context context) {
		...
		android.util.Log.i(TAG, wifiManager.isWifiEnabled() ? "WIFI已开启" : "WIFI已关闭");
	...
	public static boolean isWifiConnected(Context context) {
		...
		Log.i(TAG, connected ? "WIFI已连接上" : "WIFI尚未连接");
		return connected;
	}
	...
	public static boolean isMobileConnected(Context context) {
		...
		Log.i(TAG, connected ? "MOBILE已连接上" : "MOBILE尚未连接");
		return connected;
	}
	...
	public static int getSimOperatorInt(Context context) {
		...
		android.util.Log.i(TAG, "运行商编号 --> " + operator);
	...
	public static String getIpAddress() {
		try {
			...
			android.util.Log.e("X",ex.getMessage());
		}
		return "";
	}

	public static String getIpAddress(Context context) {
		return Build.VERSION.SDK_INT >= 14 ? getIpAddress()
				: isWifiConnected(context) ? getIpAddressFromWifi(context) : getIpAddressFromPhone();
	}
	...
	public static String getApnProxy(Context context) {
		...
		android.util.Log.i(TAG, "proxy=" + proxy);
		android.util.Log.i(TAG, "c==null:" + (c == null));
		return proxy;
	}
	...
	public static int getApnProxyPort(Context context) {
		...
		android.util.Log.i(TAG, "port=" + port);
		android.util.Log.i(TAG, "c==null:" + (c == null));
		return port;
	}
	...
	public static String getProxyHost(Context context) {
		...
		android.util.Log.i(TAG, "proxyHost=" + proxyHost);
	...
	public static int getProxyPort(Context context) {
		...
		android.util.Log.i(TAG, "proxyPort=" + proxyPort);
	...
	private static void initNetType(Context context) {
		...
		if (isWifiConnected(context)) {
			...
			if (null != networkInfo) {
				android.util.Log.e(TAG, networkInfo.getExtraInfo());
				networkType = networkInfo.getExtraInfo();
				String proxyHost = Proxy.getDefaultHost();
				if (null != proxyHost) {
					android.util.Log.e(TAG, proxyHost);
	...
	public static boolean isCDMA(Context context) {
		...
		if (null != tm && tm.getPhoneType() == TelephonyManager.PHONE_TYPE_CDMA) {
			android.util.Log.e(TAG, tm.getSimOperatorName());
	...
	public static boolean haveSIM(Context context) {
		...
		android.util.Log.e(TAG, mString);
		return true;
	}
}
